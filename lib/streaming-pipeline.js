...
module.exports = class StreamingPipeline extends Writable {
    ...
    constructor(userFunction, grpcStream, options={}) {
        ...
        this.on('pipe', (src) => {
            src.on('end', () => {
                logger('Source finished - ending all unmarshallers now');
                this._endInputs();
            });
            src.on('error', (err) => {
                // do not propagate marshaller errors to the unmarshallers
                if (err.type !== errors.types.INVALID_OUTPUT) {
                    logger('Source errored - forwarding errors to all unmarshallers now');
                    this._errorInputs(err);
                }
            });
        });
        this.on('error', (err) => {
    ...
    _errorInputs(err) {
        this._functionInputs.forEach((fi) => fi.emit('error', err));
    }
    ...
    _write(inputSignal, _, callback) {
        ...
        if (inputSignal.hasStart()) {
            ...
            if (transformerCount !== inputCount) {
    ...
    _wireInputs(transformers) {
        ...
        for (let i = 0; i < inputCount; i++) {
            ...
            unmarshaller.on('error', (err) => {
                logger(`Logical input #${i} errored - ${err.toString()}`);
                this.emit('error', err); // centralize error handling
            });
    ...
    _wireOutputs(outputContentTypes) {
        ...
        for (let i = 0; i < outputCount; i++) {
            ...
            marshaller.on('error', (err) => {
                logger(`Logical output #${i} errored - forwarding to main output now`);
                this._destinationStream.emit('error', err);
            });
            marshaller.on('finish', () => {
                this._finishedOutputs++;
                if (this._finishedOutputs === this._functionOutputs.length) {
                    logger('Last output stream closing: ending main output now');
