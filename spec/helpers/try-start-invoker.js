...
module.exports = async (functionUri, shutdownSignal, options) => {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        try {
            const {userFunction, shutdownPromise} = await startInvoker(functionUri, invokerOptions, shutdownSignal);
            return {address: `localhost:${port}`, userFunction, shutdownPromise};
        } catch (err) {
            lastError = err;
        }
    }
