...
const validateArgumentTransformer = (argumentTransformer, transformerIndex) => {
    ...
    if (transformerType !== "function") {
        throw new RiffError(
            ...
            `Argument transformer #${1 +
                transformerIndex} must be a function. Found: ${transformerType}`
        );
    }
    ...
    if (transformerArity !== 1) {
        throw new RiffError(
            errorTypes.ARGUMENT_TRANSFORMER,
            `Argument transformer #${1 +
                transformerIndex} must be a single-parameter function. Found: ${transformerArity} parameter(s)`
        );
    }
};
...
module.exports = argumentTransformers => {
    if (!Array.isArray(argumentTransformers)) {
        throw new RiffError(
            errorTypes.ARGUMENT_TRANSFORMER,
            `Argument transformers must be declared in an array. Found: ${typeof argumentTransformers}`
        );
    }
