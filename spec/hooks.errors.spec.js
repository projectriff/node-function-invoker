...
describe('hook utilities =>', () => {
    ...
    describe('timeout guard =>', () => {
        ...
        it('rejects wrongly typed hooks', async () => {
            await expectAsync(guardWithTimeout('not a valid hook', 100))
                .toBeRejectedWith(
                    new RiffError('error-invalid-hook', 'Hooks must be functions, found: string')
                );
        });
        ...
        it('rejects asynchronously throwing hooks', async () => {
            ...
            await expectAsync(guardWithTimeout(offTheHook, 100))
