...
module.exports = class StreamingPipeline extends Writable {
    ...
    constructor(userFunction, destinationStream, options) {
        super(options);
        StreamingPipeline._validateArity(userFunction["$arity"]);
    ...
    _invoke(callback) {
        try {
            const inputStreams = StreamingPipeline._toObject(this._functionInputs);
            const outputStreams = StreamingPipeline._toObject(this._functionOutputs);
    ...
    static _validateArity(arity) {
        if (typeof arity === 'undefined') {
            throw new RiffError('error-function-arity-unknown', 'Cannot determine function arity. Aborting now');
        }
        if (typeof arity !== 'number' || Math.floor(arity) !== arity || arity <= 0) {
            throw new RiffError('error-function-arity-invalid', `Function arity must be an integer >= 1, received: ${arity}. Aborting now`);
        }
    }
};
