...
const toObject = (parameters, streamNames) => {
    ...
    for (let i = 0; i < parameters.length; i++) {
        ...
        result[streamName]= parameters[i];
    }
    return result;
};
...
module.exports = class StreamingPipeline extends Writable {
    ...
    constructor(userFunction, destinationStream, streamMetadata, options) {
        super(options);
        ...
        this._arity = streamMetadata['inputs'].length + streamMetadata['outputs'].length;
