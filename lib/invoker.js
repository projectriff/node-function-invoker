...
const registerShutdown = (destroyHook, timeout, server, shutdownSignal) => {
    return shutdownSignal
        .then(() => {
            ...
            const gracefulShutdown = util.promisify(server.tryShutdown).bind(server);
            return gracefulShutdown();
        })
        .then(
            () => executeDestroy(destroyHook, timeout),
            (shutdownErr) => {
                logger(`Error during graceful shutdown: ${shutdownErr.toString()}`);
                return executeDestroy(destroyHook, timeout)
                    .then(
...
const executeInit = async (initHook, initTimeout) => {
    ...
    return guardWithTimeout(initHook, initTimeout)
        .then(() => logger('$init hook successfully completed (or skipped if none)'))
        .catch((err) => {
...
const executeDestroy = async (destroyHook, timeout) => {
    ...
    return guardWithTimeout(destroyHook, timeout)
        .then(() => logger('$destroy hook successfully completed (or skipped if none)'))
        .catch((err) => {
...
module.exports = async (userFunctionUri, options, shutdownSignal) => {
    ...
    const shutdownPromise = registerShutdown(destroyHook, destroyHookTimeout, server, shutdownSignal);
    ...
    return executeInit(userFunction.$init, initHookTimeout, server)
        .then(
            () => {
                ...
                server.bind(`0.0.0.0:${options.port}`, grpc.ServerCredentials.createInsecure());
                server.start();
            ...
            (initErr) => {
                ...
                return executeDestroy(destroyHook, destroyHookTimeout)
                    .then(
