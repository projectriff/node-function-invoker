        ...
        if (inputSignal.hasStart()) {
            ...
            if (this._startReceived) {
                callback(new RiffError(
                    ...
                    `Rejecting start signal with: output content types: [${outputContentTypes.join(",")}], ` +
                    `input names: [${this._streamMetadata.inputs.join(",")}], ` +
                    `output names: [${this._streamMetadata.outputs.join(",")}]`));

                return;
            }
            this._streamMetadata.inputs = startSignal.getInputnamesList();
            this._streamMetadata.outputs = startSignal.getOutputnamesList();
            ...
            logger(`Start signal with: output content types: [${outputContentTypes.join(",")}], ` +
                `input names: [${this._streamMetadata.inputs.join(",")}], ` +
                `output names: [${this._streamMetadata.outputs.join(",")}]`);
