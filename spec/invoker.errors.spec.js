...
describe('invoker =>', () => {
    ...
    describe('with a function with a too slow $init =>', () => {
        it('shuts down the server', async () => {
            const fnUri = '../spec/helpers/hooks/slow-init-hook-streaming-function';
            ...
            expect(grpc.Server.prototype.forceShutdown).toHaveBeenCalledTimes(1);
    ...
    describe('with a server that fails to shut down gracefully =>', () => {
        it('fails the shutdown', async () => {
            spyOn(grpc.Server.prototype, 'tryShutdown').and.throwError('tryShutdown KO');
            const shutdownTrigger = new DeferredPromiseWrapper();
            ...
            await expectAsync(shutdownPromise).toBeRejectedWith(new Error('tryShutdown KO'));
            expect(userFunction.$init).toHaveBeenCalledTimes(1);
